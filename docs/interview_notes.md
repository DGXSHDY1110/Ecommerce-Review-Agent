...
## V2-M3 补充话术：数据策略与真实感样本 (Data Strategy & Realistic Samples)

### 为什么第一版用 Mock 数据

> 面试标准回答（中文）：

"第一版用 Mock Review 数据，不是因为懒，是因为这是 AI 应用工程的标准实践：

**第一，开发解耦。** Mock 数据让我在开发 FastAPI、Guardrails、n8n 编排、Prompt Engineering 时，不需要每次调用都去真实 API。开发速度提高 10 倍，反馈时间从 30 秒降到 0 毫秒。

**第二，确定性测试。** Mock 模式的输出是 100% 确定的——同样的输入永远得到同样的输出。这保证了 CI 中的 179 个测试每次都得到相同的结果。如果用真实 LLM，今天的测试过了、明天可能因为模型微调或网络抖动失败——这种'假阳性'对工程团队是灾难。

**第三，面试演示可控。** 面试时如果网络不好、API key 过期、DeepSeek 服务降级——用 Mock 数据演示能保证流程完整跑通。这展示的是系统架构能力，不是'今天我运气好不好'。

**第四，成本可控。** 真实 LLM 调用需要 token 费用。开发阶段每天可能跑几十次、上百次。用 Mock 模式开发，只在最终验证时才用真实 LLM——这是工程成本意识。"

### 为什么 V2 加入真实感评论样本

> 面试标准回答（中文）：

"V2-M3 加入了 42 条真实感评论样本，不是为了证明数据有多好，而是为了回答面试官的一个核心问题：'这个系统真的能处理跨境电商的真实评论吗？'

Mock 数据的 10 条评论虽然覆盖了 8 个类别、5 个平台，但太规则化——每条都像手工编写的测试用例。真实跨境电商的评论是：拼写错误、多语言混用（一个德国人用英语写但夹杂德语词）、极简评论（'还行'）、情绪矛盾（低分好评）、多问题堆叠（电池差 + 物流慢 + 客服不理人）。

V2-M3 的样本模拟了这些复杂性，但它明确声明为 **public-review-style / sanitized demo sample**——不是声称抓取自真实平台，而是展示了系统架构能处理这些边缘情况。在面试时，这比 10 条干净的玩具数据有说服力得多。"

### 生产环境如何接入真实数据

> 面试标准回答（中文）：

"当前版本是 MVP，数据源是 CSV 文件。但整个架构在设计时就考虑了生产数据源的接入：

**Amazon SP-API (Selling Partner API)：**
- `GET /reviews/v1/reviews/{reviewId}` 可以拿到真实评论
- 需要 Amazon Seller Central 账号和 IAM 授权
- 评论文本、评分、评论时间、国家/地区都有
- n8n HTTP Request 节点可以直接调用 SP-API

**Shopify Admin API：**
- GraphQL `productReviews` 查询
- 需要 Shopify API access token
- 可以批量拉取评论

**飞书多维表格：**
- 运营团队可能在飞书多维表格中手动整理用户反馈
- 飞书开放平台 API 可以读取多维表格数据
- n8n 飞书节点直接集成

**客服工单系统（Zendesk、Intercom 等）：**
- 用户投诉和差评经常先出现在工单系统
- API 拉取后统一分析

关键点：数据源是可替换的——换一个数据源，FastAPI 的 `/api/v1/analyze` 接口不需要改动。这就是分层架构的好处。"

### 如何避免使用真实用户隐私数据

> 面试标准回答（中文）：

"做 AI 应用工程必须考虑数据合规，尤其是跨境电商涉及全球用户：

**第一，PII 脱敏。** `scripts/prepare_real_reviews.py` 脚本可以做字段清洗——如果原始数据包含邮箱、电话、地址，可以在清洗阶段用正则过滤或替换为占位符。

**第二，不存储原始评论。** 在生产环境中，review_text 只用于分析，不长期存储。分析完成后只保留结构化结果（类别、情绪、摘要）和脱敏后的 evidence 片段。原始评论如果有 PII，分析结果也不能包含 PII。

**第三，Prompt 约束。** Prompt 已明确要求模型'只能根据 review_text 判断，不允许编造外部事实'。evidence 字段要求 verbatim 引用原文——如果原文包含 PII，模型不应该在 evidence 中重复 PII。这可以通过 Prompt 进一步约束或在 Guardrails 层做 post-hoc 过滤。

**第四，区域合规。** 欧洲 GDPR、加州 CCPA、中国个人信息保护法——不同的用户数据需要不同的存储和删除策略。MVP 阶段本地 CSV 不涉及这个问题，但生产部署时必须考虑。

**第五，面试演示数据。** V2-M3 的 42 条样本完全不包含真实个人信息——没有真实姓名、没有真实邮箱、没有真实订单号。它是 based on 公开的产品评论写作风格，但内容是全新编写的。这在面试中完全可以大方说明。"

### 如何用 --limit 控制真实 LLM 调用成本

> 面试标准回答（中文）：

"真实 LLM API 调用不是免费的。DeepSeek V4 Pro 按 token 计费——每条评论可能消耗 500-1000 tokens（含 Prompt + 输出），42 条评论就是 2-4 万 tokens。开发阶段不需要每次都跑全套。

`--limit` 参数是这个项目的一个小但重要的设计：

```bash
# 只分析 3 条，验证 Prompt 效果
python scripts/run_batch.py --limit 3

# Mock 模式也支持 --limit，快速验证新数据
python scripts/run_batch.py --mock --limit 5
```

这在面试时能展示成本意识：**不是有了 API key 就无脑跑全套，而是根据场景选择合适的数据量和模式。** Mock 开发 → 真实小批量验证 → 真实全量跑批——这是一个完整的低成本到高保真度的开发流程。"

### 为什么真实 LLM 分析比关键词 Mock 更可信

> 面试标准回答（中文）：

"面试官问：'你都有 mock 了，为什么还要真实 LLM？关键词分类不也挺准的吗？'

答案：关键词分类有根本性的局限性。

**第一，无法处理语义。** 'The camera wasn't what I expected at all'——关键词匹配找不到 battery、logistics、price 等任何词，只能归到 other。但真实 LLM 可以从 'not what I expected' 推断出可能是 description_mismatch 或 product_quality 问题。

**第二，无法处理多语言。** 关键词是英文的——德语、日语、法语评论全部归到 other。但 DeepSeek V4 Pro 是多语言模型，可以直接理解并输出中文摘要。

**第三，无法生成自然摘要。** V2-M2 改进后的 Mock 摘要仍然是模板化中文——'客户对X不满意（有较详细描述），给出Y星低分。' 面试官读两三条就能看出是模板。真实 LLM 的摘要：'客户反映电池耗电过快，夜视效果模糊，认为产品不值这个价格。'——这是自然语言。

**第四，置信度更真实。** Mock 的 confidence 只有 0.50、0.80、0.85 三档，是人为定义的。真实 LLM 的 confidence 可以精确到 0.88、0.92、0.67——更细粒度的不确定性表达。

**第五，证据更准确。** Mock 的 evidence 是通过关键词窗口提取的 fragment。真实 LLM 可以理解整句话的语义，选取最相关的 verbatim 片段作为证据。

面试展示时，用真实 LLM 跑 3 条评论，当场看到从原文到结构化 JSON（含中文摘要、证据片段）的完整链路——比任何 PPT 都有说服力。"
